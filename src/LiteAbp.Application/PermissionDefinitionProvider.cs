...
namespace LiteAbp.Application
{
    public class PermissionDefinitionProvider : Volo.Abp.Authorization.Permissions.PermissionDefinitionProvider
    {
        public override void Define(IPermissionDefinitionContext context)
        {
            ...
            {
                var group = context.AddGroup(groupPermissionName, LocalizableString.Create<AppResource>($"Permission:{groupPermissionName}"));
                foreach (var permissionName in permissionNames.Where(x => x.StartsWith(groupPermissionName + ".") && x.Split('.').Length == 2))
                {
                    var permission = group.AddPermission(permissionName, LocalizableString.Create<AppResource>($"Permission:{permissionName}"));
                    foreach (var childPermissionName in permissionNames.Where(x => x != permissionName && x.StartsWith(permissionName + ".")))
                    {
                        permission.AddChild(childPermissionName, LocalizableString.Create<AppResource>($"Permission:{childPermissionName}"));
                    }
                }
            }
        }
